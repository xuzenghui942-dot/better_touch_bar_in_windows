...
- 独立单指移动、点击和抬起逐帧立即交给克隆触摸板，不进入缓存、标题栏探测或窗口手势状态机；
- 只有恰好两指出现后的候选帧才暂停送给浏览器/GTK。GNOME broker 先在标题栏内锁定窗口，Rust 与 Windows 版使用同一套方向、按住、捏合和轴缩放状态机；识别期间不会模拟左键，也不会让指针跟着手指移动；
- 关闭“实时预览”时显示目标区域 HUD，窗口只在松手后移动；开启时真实窗口会在选区间预览，取消则还原；
- “鼠标跟随窗口”是独立可选项：关闭时指针原地不动；开启时只在最终吸附后按窗口移动前的相对像素偏移重定位，并不跟随双指轨迹；
- 恰好两指出现后最多暂存 64 帧/50 ms；非标题栏、探测超时、回到一指或第三/四指加入时，按原顺序回放候选帧并补发当前边界帧，普通双指滚动仍由 libinput 连续处理；
- 三指由拖动后端所有，四指会重建到虚拟触摸板并保持由 GNOME 处理；只有启用的“四指向下全部最小化”会在手势完整结束后附加执行；
- 任何 uinput 写入、D-Bus 或输入状态不可确认都会取消窗口事务并立即解除物理设备独占；协议/构建不匹配则在 `EVIOCGRAB` 之前退出高级模式。
