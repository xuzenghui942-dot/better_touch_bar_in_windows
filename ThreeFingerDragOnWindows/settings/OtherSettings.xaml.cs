    ...
    private void UpdateStartupStatus(){
        ...
        if(App.SettingsData.RunElevated){ // Elevated startup task
            ...
            if(StartupManager.IsElevatedStartupOn()){
                ...
                if(Utils.IsAppRunningAsAdministrator()){
                    StartupStatus.Title = "已设置为开机启动，并已配置跳过 UAC 提示。";
                } else{
                    StartupStatus.Title =
                        "已设置为开机启动，并已配置跳过 UAC 提示。关闭此选项时，应用需要以管理员权限重新启动。";
                }
                ...
                if(Utils.IsAppRunningAsAdministrator()){
                    StartupStatus.Title =
                        "当前未设置为开机启动。可在此启用，并配置为跳过 UAC 提示。";
                    StartupStatus.Severity = InfoBarSeverity.Informational;
                } else{
                    StartupStatus.Title =
                        "当前未设置为开机启动。可在此启用并配置为跳过 UAC 提示，但应用需要以管理员权限重新启动。";
            ...
            DispatcherQueue.TryEnqueue(() => {
                switch(task.Result){
                    case StartupTaskState.Enabled:
                        StartupStatus.Title = "已设置为开机启动。";
                        StartupStatus.Severity = InfoBarSeverity.Success;
                        break;
                    case StartupTaskState.Disabled:
                        StartupStatus.Title = "当前未设置为开机启动。";
                        StartupStatus.Severity = InfoBarSeverity.Informational;
                        doTurnOnStartupToggle = false;
                        break;
                    case StartupTaskState.DisabledByUser:
                        StartupStatus.Title =
                            "启动任务已在任务管理器中被停用，因此应用无法开机启动。请在任务管理器中重新启用，然后重新打开此页面。";
                        StartupStatus.Severity = InfoBarSeverity.Warning;
                        doTurnOnStartupToggle = false;
                    ...
                    case StartupTaskState.EnabledByPolicy:
                        StartupStatus.Title =
                            "启动任务已由组策略启用。你仍可配置管理员启动任务，让应用以管理员身份运行。";
                        StartupStatus.Severity = InfoBarSeverity.Warning;
                        doTurnOnStartupToggle = false;
                        break;
                    case StartupTaskState.DisabledByPolicy:
                        StartupStatus.Title =
                            "启动任务已被组策略停用。你仍可配置管理员启动任务，让应用以管理员身份运行。";
