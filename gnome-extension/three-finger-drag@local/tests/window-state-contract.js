...
class FakeAnimationActor {
    constructor(window) {
        ...
        this._transitionIds = new Set();
    }

    get_transformed_position() { return [this.window.frame.x, this.window.frame.y]; }
    get_transformed_size() { return [this.window.frame.width, this.window.frame.height]; }
    ...
    remove_all_transitions() {
        this.cancelCalls++;
        for (const id of this._transitionIds)
            GLib.source_remove(id);
        this._transitionIds.clear();
    }
    ease(params) {
        this.easeCalls++;
        let id = 0;
        id = GLib.timeout_add(GLib.PRIORITY_DEFAULT, params.duration, () => {
            this._transitionIds.delete(id);
...
for (const startingFlags of [
    ...
    for (const zone of ['leftHalf', 'rightHalf', 'topLeft', 'bottomRight']) {
...

{
    ...
    assert(hud.badges === 0,
        'normal direction updates must stay completely visual-free');
...

{
    ...
    backend._settings.snapAnimationSeconds = 0.05;
...

{
    ...
    backend._mode = 'swipe';
    assert(backend.handleUpdate({kind: 'updated', direction: 'left', progress: 1}),
        'live preview update must be accepted');
    runUntil(() => target.geometryWrites === 2);
    assert(target.frame.x === 0 && target.frame.width === 960,
        'live preview must move the real window to the selected zone');
}
